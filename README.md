...
Official site: https://hyperplan.dev/modinet
...
- Relay 모드에서는 `agentRelayAllowedOrigins`를 설정하면 브리지 주입 대상(origin)이 runtime에 자동 반영됩니다.
- 상세 스펙은 `docs/AGENT_AUTOMATION.md`를 참고하세요.
- WebSocket Relay 방식(방법 2)은 `docs/WEBSOCKET_RELAY.md`를 참고하세요.
- Relay 에이전트 실전 가이드(KO): `docs/RELAY_AGENT_GUIDE.ko.md`
- Relay agent practical guide(EN): `docs/RELAY_AGENT_GUIDE.en.md`
- 로컬 검증용 실행 스크립트: `npm run relay:server`, `npm run relay:page`
...
- 20개 초과 생성 시 라이선스 재검증은 **7일 주기**로 수행됩니다.
- 라이선스 서버가 일시 장애(네트워크/5xx)인 경우, 기존 유효 라이선스 사용자에게는 **1일 grace**가 적용됩니다.
- 서버가 유효하지 않은 키를 명시적으로 반환하면 즉시 차단됩니다.
